...
void pipe_cycle()
{
    ...
    if (IC_STALL_CYCLE==9) {    // start of 10th icache stall cycle
        DO_IF = 0x1;
    }
...
void pipe_stage_wb() {
    ...
    if (DC_RUN_CYCLE==2) {
        // avoid repeat processing dcache miss on icache miss bubble
    ...
    if (is_ld||is_st) {
	    ...
	    if (!DC_STALL) {
		    ...
		    if (!dcache_hit) {	                   // MISS
                ...
                DCMISS_ICMISS_DIFF = IC_STALL_CYCLE;
			    DC_STALL = 0x1;
				DC_STALL_CYCLE = 1;
				DC_RUN_CYCLE = 0;
                // backup EXM
...
void pipe_stage_fetch() {
	...
	if (IC_STALL_CYCLE==9) { // earlier cache miss now resolved
